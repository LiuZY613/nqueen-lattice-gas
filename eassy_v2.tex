...
point for $N = 8$ to $1024$ reveal that the specific heat per queen
...
Monte Carlo data, providing the first \lw{no need to claim first} \emph{physical} determination
of this fundamental combinatorial constant.
Thermodynamic integration yields
$\gamma_{\rm MC} = 1.946$ at $N = 1024$, within $0.1\%$ of the
precise combinatorial value.
\end{abstract}
...
This \lw{it is unclear what this "is" across the paragraph} observation motivates the present work.  We embed the $N$-queens
...
that have dominated to date~\cite{Simkin2022}.  Our thermodynamic extraction yields $\gamma_{\rm MC} = 1.946$ at
...
(queen--vacancy exchange)  \lw{why this ? why not queen-queen exchange ?} with the Metropolis acceptance
...
The simulations were carried out using 260 CPU cores in parallel,
...
\begin{figure}[t]
  ...
  (a)~Acceptance rate of the Kawasaki (queen--vacancy exchange) \lw{consider making this log y scale} moves
  as a function of temperature for $N = 8$ to $1024$.  The rate drops
  below $10^{-5}$ for $T \lesssim 0.1\,J$ and rises monotonically,
  reaching $0.38$--$0.47$ at $T = 1\,J$.
  ...
  sweeps.
  \lw{legend (b) overlapped with curve}
  }
...
Figure~\ref{fig:energy} shows $E/N$ vs.\ $T$ for $N = 8$ to $1024$.
...
\begin{figure}[t]
  ...
  $N = 8$ to $1024$.  (a)~Logarithmic temperature scale over the full
...
\begin{figure}[t]
  ...
  the inset enlarges the peak vicinity ($T = 0.19$--$0.23\,J$) for
  $N \ge 64$, revealing subtle size-dependent shifts in the peak
  height and position.
...
\begin{table}[b]
  \caption{$C_v/N$ peak scaling from dedicated high-resolution simulations
  at $N = 8$ to $128$ with dense temperature sampling in $T \in (0.20,\,0.30)\,J$.
  The peak height saturates near $1.63$ for $N \ge 32$, ruling out a
...
between $N = 128$ and $N = 1024$ is only $\Delta C_v^{\max}/N
...
$1.91$ ($N = 128$), $1.93$ ($N = 256$),
$1.93$ ($N = 512$), and $1.946$ ($N = 1024$), with the deviation
...
\begin{table}[t]
  ...
  \begin{tabular}{cD{.}{.}{3}D{.}{.}{3}D{.}{.}{3}c}
    ...
    & Deviation \\
    \hline
    8   & 0.566 & 0.565 & \multicolumn{1}{c}{---} & 0.1\% \\
    16  & 1.031 & 1.032 & \multicolumn{1}{c}{---} & 0.1\% \\
    \noalign{\smallskip}\hline\noalign{\smallskip}
    32  & 1.633 & \multicolumn{1}{c}{---} & 1.833 & 5.7\% \\
    64  & 2.274 & \multicolumn{1}{c}{---} & 1.885 & 3.0\% \\
    128 & 2.943 & \multicolumn{1}{c}{---} & 1.909 & 1.8\% \\
    256 & 3.620 & \multicolumn{1}{c}{---} & 1.925 & 1.0\% \\
    512 & 4.308 & \multicolumn{1}{c}{---} & 1.931 & 0.69\% \\
    1024 & 4.985 & \multicolumn{1}{c}{---} & 1.946 & 0.11\% \\
  \end{tabular}
  \end{ruledtabular}
\end{table}
...
We have presented a statistical-mechanical investigation of the
$n$-queens lattice gas \lw{call the problem n-queens lattice gas} through
extensive Monte Carlo simulations.  Our main findings are:
...
\item The specific heat per queen $C_v/N(T)$ converges to a universal
  function of $T$ as $N \to \infty$.  The peak height $\approx 1.63$
...
\item Thermodynamic integration of $C_v/T$ recovers the ground-state
  ...
  agreeing with exact values at $N = 8$ and $16$ to within $0.1\%$.
  For $N \ge 32$, the extracted Simkin constant
  $\gamma_{\rm MC} = \ln N - s_0^{\rm MC}$ converges
  to $\gamma = 1.944001$~\cite{Nobel2023}, reaching $1.946$ at $N = 1024$ ($0.11\%$
...
These results establish the queen lattice gas as a model system
in which the interplay between combinatorial and thermodynamic
quantities can be exploited quantitatively: a single Monte Carlo
campaign, combined with exactly known high-temperature entropy,
suffices to determine a fundamental combinatorial constant through
the laws of thermodynamics.

Extending to still larger system sizes ($N \gtrsim 1024$) would
further sharpen the thermodynamic determination of $\gamma$,
potentially bringing the Monte Carlo estimate to sub-$0.1\%$
accuracy and rivaling direct combinatorial methods.
