...
$s_0 \approx \ln N - \gamma$ ($\gamma \in [1.939,\, 1.945]$) exhibits a
...
algorithm.  All of these determinations are purely
...
that have dominated to date~\cite{Simkin2022}.  Our thermodynamic extraction yields $\gamma_{\rm MC} = 1.94$ at
...
\begin{equation}
  ...
  = \ln N - \gamma, \quad \gamma \in [1.939,\, 1.945].
  \label{eq:s0}
\end{equation}
...
asymptotic value $\gamma \in [1.939,\, 1.945]$~\cite{Simkin2022}:
...
The monotonic convergence of $\gamma_{\rm MC}$ toward $[1.939,\, 1.945]$ with
...
\begin{table}[t]
  ...
  Simkin asymptotic value $\gamma \in [1.939,\, 1.945]$.
  The $N = 8$--$256$ data use a 280-point temperature grid;
  $N = 512$ uses a 126-point grid optimized for the
  ...
  \begin{tabular}{cD{.}{.}{3}D{.}{.}{3}D{.}{.}{3}c}
    ...
    \hline
    32  & 1.633 & \multicolumn{1}{c}{---} & 1.833 & 5.6\% \\
    64  & 2.274 & \multicolumn{1}{c}{---} & 1.885 & 2.9\% \\
    128 & 2.943 & \multicolumn{1}{c}{---} & 1.909 & 1.7\% \\
    256 & 3.620 & \multicolumn{1}{c}{---} & 1.925 & 0.9\% \\
    \hline
...
\item Thermodynamic integration of $C_v/T$ recovers the ground-state
  ...
  to $\gamma \in [1.939,\, 1.945]$, reaching $1.94$ at $N = 512$ ($0.07\%$
