...
with each core handling all seven system sizes at a single temperature
...
\begin{figure}[t]
  ...
  as a function of temperature for $N = 8$--$1024$.  The rate drops
...
\begin{figure}[t]
  ...
  $N = 8$--$1024$.  (a)~Logarithmic temperature scale over the full
...
\begin{table}[b]
  ...
  at $N = 8$--$128$ with dense temperature sampling in $T \in (0.20,\,0.30)\,J$.
...
\end{equation}
within $1.8\%$ of the Monte Carlo value $\approx 1.625$ for $N \ge 128$.
...
Carlo data ($N = 128$ and $256$).  The modified Poisson MF
...
\begin{figure}[t]
  ...
  the largest MC data are shown, as these are closest to
...
We have presented a statistical-mechanical investigation of the n-queens
lattice gas  \lw{call the problem n-queens lattice gas} combining extensive Monte Carlo
...
\item A modified Poisson mean-field theory, treating each attack line
  ...
  [$C_v^{\max} = 4(1+\gamma)/e^2 \approx 1.593$] achieves $\sim 2\%$ accuracy at the
