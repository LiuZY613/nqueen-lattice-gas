...
sizes.
All of these determinations are purely combinatorial.  Equation~\eqref{eq:QN} yields an entropy per queen
...
Section~\ref{sec:conclusions} concludes with a discussion of the implications for connecting combinatorial and thermodynamic information, and possible extensions via tensor network methods.
...
solution count $Q(N)$.  More generally, tensor network contraction
provides a powerful framework for exact counting in constraint
satisfaction problems~\cite{Kourtis2019}, of which the $N$-queens
problem is a natural instance.
The contraction can be performed row by row
using matrix product operator methods~\cite{Xiang2024}, with a
...
\lw{swap the order of the following two figures}

Figure~\ref{fig:tn_network} illustrates the complete tensor network
for a $4\times 4$ board. \lw{this is a paragraph with a standalone sentences}

\begin{figure}[t]
  \includegraphics[width=\linewidth]{figures/fig7_tn_schematic.pdf}
  \caption{Tensor network for exact counting of $N$-queens solutions,
  shown for $N = 4$.  Each node represents the rank-9 site tensor~$T$
  [Eq.~\eqref{eq:Ttensor}] with a physical index~$p$ (gray stubs,
  summed over $\{0,1\}$).  Solid red lines are row bonds and solid
  blue lines are column bonds; dashed green lines carry
  $\searrow$-diagonal signals and dotted purple lines carry
  $\nearrow$-diagonal signals.  Boundary vectors are attached at
  every constraint-line terminus: $\mathbf{v}_0 = (1,0)$ initializes
  each line (no queen seen yet), $\mathbf{v}_1 = (0,1)$ terminates
  rows and columns (exactly one queen required), and
  $\mathbf{v}_2 = (1,1)$ terminates diagonals (at most one queen
  allowed).  Contracting the full network---summing over all bond and
  physical indices---yields the exact solution count $Q(N)$.  The
  arrow indicates the row-by-row contraction order used in the matrix
  product operator approach. \lw{what happens to the physical indices ?}}
...
the corner transfer matrix renormalization group (CTMRG)~\cite{Nishino1996} can
approximate this limit at moderate bond dimension; such methods have
been successfully applied to compute residual entropies in
...
GitHub~\cite{github_repo}. \lw{this is fine: Z.-Y. Liu, Simulation code and
data for the N-queens lattice gas (we did not made contribution to the simulation code,.)}
