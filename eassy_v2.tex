...
We further present a tensor network formulation that encodes the
non-attacking constraints into a rank-9 site tensor with 17 nonzero
elements, providing a complementary exact-enumeration route.
\end{abstract}
...
%=====================================================================
\section{Outlook: Exact enumeration via tensor networks}
\label{sec:tensor}
%=====================================================================

The Monte Carlo thermodynamic integration developed above provides a
powerful route to the Simkin constant, but is inherently limited by
statistical noise and finite-size corrections---as seen in
Table~\ref{tab:entropy_check}, the extracted $\gamma_{\rm MC}$
approaches the exact value only as $N \to \infty$.
Tensor network methods offer a complementary, deterministic route
to the exact solution count $Q(N)$ by reformulating the constraint
satisfaction problem as a tensor network
contraction~\cite{Liu2021,Pan2022}.

The construction places a rank-9 tensor $B$ at each lattice site
$(i,j)$.  Eight of its indices carry binary attack signals along the
four cardinal and four diagonal directions; a ninth physical index
$p \in \{0,1\}$ encodes the occupation state.  The tensor has exactly
17 nonzero elements (Fig.~\ref{fig:tensor}): for an empty site
($p = 0$), the four independent attack channels---column, row, and
two diagonal families---can each independently pass through or block
an incoming signal, yielding $2^4 = 16$ allowed local states; for an
occupied site ($p = 1$), the unique nonzero element requires all
incoming signals to be absent (no prior queen attacks this square)
and all outgoing signals to be present (the queen attacks along every
direction).  Summing over $p$ at every site and contracting the full
network yields the exact count $Q(N)$.

The diagonal connections render the raw network non-planar.  To
restore a planar square-lattice topology amenable to matrix product
operator (MPO) contraction, auxiliary SWAP tensors encode the two
diagonal signals into the horizontal and vertical bond
indices~\cite{Liu2021}.  The resulting network can be contracted row
by row using standard MPS--MPO algorithms, with a computational cost
that grows exponentially in $N$ through the bond dimension but
involves no statistical sampling.  This tensor network formulation
thus complements the Monte Carlo approach: while the latter accesses
arbitrarily large $N$ with controlled statistical errors, the former
provides exact results for moderate $N$ and can serve as a benchmark
for the Monte Carlo data.

\begin{figure}[t]
  \includegraphics[width=\linewidth]{figures/fig6_tensor.png}
  \caption{The 17 nonzero elements of the site tensor $B$.
  Each diagram shows a lattice site with eight directional indices
  carrying binary attack signals ($0$: absent, $1$: present);
  red arrows mark the directions carrying an active signal.
  The first 16 elements correspond to empty sites ($p = 0$) with
  all $2^4$ combinations of pass-through signals along four
  independent attack channels (column, row, and two diagonal
