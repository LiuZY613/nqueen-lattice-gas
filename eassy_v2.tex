...
exactly 17 nonzero elements (Fig.~\ref{fig:tensor}).
For an empty site ($p = 0$), each of the four channels independently
passes or blocks its signal, giving $2^4 = 16$ elements.
For an occupied site ($p = 1$), all incoming signals must vanish and
all outgoing signals are emitted, giving one additional element.
Attaching boundary vectors $\bm{v}_0$ at each constraint-line origin
and $\bm{v}_1$ or $\bm{v}_2$ at each terminus, and summing over all
physical indices, yields the exact solution count $Q(N)$.
