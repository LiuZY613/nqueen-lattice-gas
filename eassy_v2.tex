...
\lw{Consider removing redundant figures (with both linear and logarithmic axes).}
...
\begin{itemize}
  ...
  \item \textit{Permutation} (row + column):
    $\Omega = N!$, $s \approx \ln N - 1$. \lw{this is called rook problem}
...
\begin{figure}[t]
  ...
  configurations. \lw{gray squares means something different. that is not good. 
  use consistent coloring schemes. It is actually possible to combine (a) and (b) into a single figure with all information}
   \lw{make this figure nicer, black dots -> queen look from above} }
  \label{fig:schematic}
\end{figure}
...
\lw{red https://people.inf.ethz.ch/markusp/teaching/guides/guide-tables.pdf 
for nice tables}
