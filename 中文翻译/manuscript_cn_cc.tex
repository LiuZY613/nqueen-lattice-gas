...
\begin{abstract}
我们从统计力学的角度研究皇后格点气体——一种将 $M$ 个皇后放置在 $N \times N$ 棋盘上、沿共享行、列和对角线具有两两排斥相互作用的模型。在单位填充（$M = N$）条件下，基态恰好对应经典 $N$-皇后问题的 $Q(N)$ 个解，每个皇后的熵为 $s_0 \approx \ln N - \gamma$（$\gamma \approx 1.942$）。我们推导出精确的高温能量 $E/M \to 5/3$，并表明它分解为 $1/2 \times 2$（行和列）$+ 1/3 \times 2$（对角线），每个贡献可追溯到独立攻击线的泊松占据统计。对 $N = 8$--$128$ 进行的大规模蒙特卡洛模拟（每个温度点 $10^8$ 次扫描）表明，每个皇后的比热 $C_v/M$ 在 $N \to \infty$ 时收敛到温度 $T$ 的普适函数，具有不发散的峰值 $C_v^{\max}/M \approx 1.62$（位于 $T^* \approx 0.237\,J$），从而确立了热力学相变的缺失。我们基于独立攻击线统计构建了修正泊松平均场理论，给出了极限情形下 $E/M(T)$ 和 $C_v/M(T)$ 的解析表达式，在 $T \ge 0.3\,J$ 时重现蒙特卡洛比热的误差在 $2\%$ 以内。平均场分解揭示，在比热峰值处，行与列贡献约 $67\%$，对角线贡献约 $33\%$。对 $C_v/T$ 从 $T = \infty$ 到 $T = 0$ 的热力学积分恢复了已知的基态熵，提供了连接组合数学、模拟和解析理论的非平凡自洽性检验。在半填充（$M = N^2/2$）条件下，能量标度为 $E \sim N^3$——与单位填充时的 $E \sim N$ 标度根本不同——系统表现出自平均性，每个皇后的能量涨落在热力学极限下消失。
\end{abstract}
...
在本文中，我们设 $k_B = 1$，使得熵为无量纲量，温度具有能量单位。值得注意的是，$s_0 = \ln N - \gamma$ 的函数形式令人联想到理想气体的 Sackur--Tetrode 每粒子熵 $s_{\rm gas} = \ln(A/N\lambda_{\rm th}^2) + 2$~\cite{Reif1965}，两者都具有"$\ln$（相空间）$+$ 常数"的结构。我们将在第~\ref{sec:entropy}~节进一步讨论这一形式上的相似性，但需要强调的是，其背后的物理机制不同：皇后系统的熵源于离散的组合约束，而非连续相空间的积分。

熵可以通过约束严格程度递增的层次来理解~\cite{Knuth2022}：
\begin{itemize}
...
\end{itemize}
列约束恰好消耗一个单位的熵（Stirling 近似修正）；两组对角线约束共消耗 $\gamma - 1 \approx 0.94$ 个单位。

一个自然的问题由此产生：皇后系统的有限温度热力学能否用解析方法理解？我们通过将 $N$-皇后问题嵌入具有可调温度的皇后格点气体~\cite{Zhang2009,Polson2024}，执行大规模蒙特卡洛模拟，并推导基于独立攻击线统计的泊松平均场理论——它提供了精度在百分之几以内的解析热力学描述——来回答这一问题。
...
第~\ref{sec:entropy}~节讨论基态熵及其约束层次。
...
我们在固定粒子数 $M = \sum_{ij} n_{ij}$ 的正则系综中工作，使用 Kawasaki 动力学~\cite{Kawasaki1966}（皇后-空位交换）和 Metropolis 准则~\cite{Metropolis1953}。一次\textit{扫描}包含 $M$ 次尝试交换。统计误差通过 200 个区间的 jackknife 重抽样估计~\cite{Efron1982}。对于 $M = N$ 的主要结果，我们模拟 $N = 8, 16, 32, 64, 100, 128$，每个温度点进行 $10^8$ 次测量扫描（318 个温度点，覆盖 $T = 0.05$ 到 $400\,J$），之前进行 $2 \times 10^6$ 次热化扫描，使用 32 个 CPU 并行计算。
我们通过从多个随机初始构型（种子）出发的独立模拟验证了遍历性；所有运行在每个温度下均给出统计一致的 $E/M$ 和 $C_v/M$ 结果，确认 Kawasaki 动力学在我们的热化和测量窗口内充分探索了构型空间。
模型示意图及相关长度尺度示于图~\ref{fig:schematic}。

\begin{figure}[t]
  \includegraphics[width=\linewidth]{../figures/fig1_schematic.pdf}
  \caption{(a)~一个皇后（黑色圆圈）及其在 $8 \times 8$ 棋盘上的四条攻击线：行（红色）、列（蓝色）、主对角线（橙色）、反对角线（紫色）。(b)~$Q(8) = 92$ 个非攻击构型之一，$M = N = 8$ 个皇后。棋盘边长 $L = N\lambda$，其中 $\lambda$ 为格点间距（一个格点宽度），定义了系统尺寸；$\ln N = \ln(L/\lambda)$ 给出了在无列和对角线约束时的单粒子组合熵。}
  \label{fig:schematic}
\end{figure}
...
\begin{equation}
  ...
  \label{eq:s_perm}
\end{equation}
因此列约束恰好消耗一个单位的熵——即来自 Stirling 近似的修正项。

两组对角线约束进一步将熵降低至 $s_0 = \ln N - \gamma$，额外消耗 $\gamma - 1 \approx 0.94$ 个单位，即每组对角线族约 $(\gamma - 1)/2 \approx 0.47$ 个单位。
...
如引言所述，$s_0 = \ln N - \gamma$ 的形式与二维 Sackur--Tetrode 结果 $s_{\rm gas} = \ln(A/N\lambda^2) + 2$ 表面上相似：两者的熵都取"$\ln$（可及态数）$+$ 常数"的形式。然而，这一类比不宜过度推广。理想气体中的常数 $+2$ 源于连续动量积分和 Gibbs $1/N!$ 修正，而 $-\gamma$ 编码的是纯粹离散的几何排斥约束。因此，这一相似性是形式上的，反映了"独立自由度经约束削减后计数"这一共同数学结构，而非深层的物理等价。
...
图~\ref{fig:energy} 展示了 $N = 8$--$128$ 的 $E/M$ 与 $T$ 的关系。$N \ge 32$ 的曲线几乎不可区分，确认了向热力学极限的快速收敛。
...
每个皇后的比热 $C_v/M = (\langle E^2\rangle - \langle E\rangle^2)/(T^2 M)$ 示于图~\ref{fig:cv}。对于 $N \ge 32$，$C_v/M$ 曲线坍缩到 $T$ 的单一函数上，表明已达到热力学极限。
...
\begin{figure}[t]
  ...
  (b)~对数 $T$ 标度下的全温度范围。
  $N \ge 32$ 的曲线坍缩到单一函数上，确认了向热力学极限的收敛。}
  \label{fig:cv}
\end{figure}

\begin{table}[b]
  \caption{$M = N$ 时 $C_v/M$ 峰值的标度行为，由四次多项式拟合结合 bootstrap 重抽样（5000 次）确定。峰值高度在 $N \ge 32$ 时趋于饱和，约为 1.62，排除了发散的感生率从而排除了连续相变。}
  \label{tab:cv_peak}
  \begin{ruledtabular}
...
关键观察是 $C_v^{\max}/M$ 在 $N \ge 32$ 时趋于饱和约 $1.62$，且\emph{不}随系统尺寸发散。这排除了热力学相变，并将该交叉行为认定为 Schottky 反常~\cite{Binder2010}——一种由有限能隙上的热激发引起的 $C_v$ 宽峰，无任何对称性破缺。峰值温度 $T^* \approx 0.237\,J$ 在 $N \ge 64$ 时稳定，峰值高度从上方单调收敛：$N = 64$ 与 $N = 128$ 之间的残余有限尺寸修正仅为 $\Delta C_v^{\max}/M \approx 0.001$，完全在统计误差棒范围内。
...
= -\tfrac{1}{2}\ln(2\pi(N-1))$ 收集了次主导对数项。
展开 $\ln(1 - 1/N) = -1/N - 1/(2N^2) - \cdots$，主导项给出 $-(N^2-N)\ln(1-1/N) = N - \tfrac{1}{2} + O(N^{-1})$，因此
\begin{equation}
  \frac{S(\infty)}{N}
  = \ln N + 1 + O\!\left(\frac{\ln N}{N}\right).
  \label{eq:Sinf}
...
将方程~\eqref{eq:DS} 重新排列，可以直接从蒙特卡洛数据提取每个皇后的基态熵：
\begin{equation}
  s_0^{\rm MC}
  = \frac{S(\infty)}{N} - \int_0^\infty \frac{C_v}{NT}\,dT,
  \label{eq:s0_MC}
\end{equation}
其中 $S(\infty)/N = \frac{1}{N}\ln\binom{N^2}{N}$ 是精确的。这提供了一种无需直接枚举即可纯热力学地确定基态简并度的方法。

数值积分使用 318 个对数等间距的温度网格点，覆盖 $T = 0.05\,J$ 到 $400\,J$；低温截断的贡献可以忽略，因为 $C_v/T$ 在 $T \approx 0.1\,J$ 以下呈指数抑制，而 $400\,J$ 的高温截断已经足够，因为 $C_v/T \propto T^{-3}$ 快速衰减，超过该点的贡献不到积分总量的 $0.01\%$。对数网格上的梯形法则引入的离散化误差远低于蒙特卡洛数据的统计不确定性。

表~\ref{tab:entropy_check} 总结了结果。
对于 $N = 8$ 和 $N = 16$，$Q(N)$ 的精确值已知（$Q(8) = 92$，$Q(16) = 14\,772\,512$），$s_0^{\rm MC}$ 与精确值 $s_0^{\rm exact} = \ln Q(N)/N$ 分别吻合到 $1.2\%$ 和 $0.5\%$ 以内，验证了模拟和积分过程的可靠性。

对于 $N \ge 32$，精确的 $Q(N)$ 未知，我们利用 $s_0^{\rm MC}$ 通过 $\gamma_{\rm MC} = \ln N - s_0^{\rm MC}$ 提取 Simkin 常数。提取的数值向已知渐近值 $\gamma = 1.942$~\cite{Simkin2022,Nobel2023} 单调收敛：$\gamma_{\rm MC} = 1.83$（$N = 32$）、$1.89$（$N = 64$）、$1.90$（$N = 100$）、$1.92$（$N = 128$），偏差从 $5.9\%$ 递减至 $1.1\%$。中等 $N$ 处误差的主要来源是 Simkin 公式的有限尺寸修正 [方程~\eqref{eq:QN} 中的 $o(1)$ 项]，它使得 $\gamma_{\rm eff}(N) = \ln N - s_0(N)$ 系统性地小于渐近值 $\gamma$；蒙特卡洛积分本身仅引入低于百分之一的误差，这已由 $N = 8$ 和 $N = 16$ 的基准测试所证实。$\gamma_{\rm MC}$ 随 $N$ 增大向 $1.942$ 的单调收敛确认了蒙特卡洛热力学、$S(\infty)$ 的 Stirling 近似以及 Simkin 组合公式三者之间的相互自洽性。

\begin{table}[b]
  \caption{通过热力学积分得到的基态熵。
  $s_0^{\rm MC} = S(\infty)/N - \int_0^\infty (C_v/NT)\,dT$，其中
  $S(\infty)/N = \frac{1}{N}\ln\binom{N^2}{N}$ 是精确的。
  对于 $N \le 16$，将 $s_0^{\rm MC}$ 与精确基态熵 $s_0^{\rm exact} = \ln Q(N)/N$ 比较。
  对于 $N \ge 32$，提取 $\gamma_{\rm MC} = \ln N - s_0^{\rm MC}$ 并与 Simkin 渐近值 $\gamma = 1.942$ 比较。}
...
残余偏差源于两种近似具有部分补偿效应：熵加权前因子 $1+\gamma \approx 2.942$ 低估了完全四族分解的贡献，而忽略线间关联则高估了能量。这种补偿并非偶然，而是反映了熵加权的物理内涵——它有效地将部分关联效应吸收到了有效相互作用方向数的减少中。
...
图~\ref{fig:mf} 将两种平均场预言与蒙特卡洛数据进行了比较。修正泊松理论系统性地高估 $E/M$ 约 $2$--$7\%$（低温端偏差较大）。如第~\ref{sec:mf}A~节所述，这一偏差是 $O(1)$ 量级的效应：每对攻击线通过共享格点产生 $O(1/N)$ 的关联，但 $O(N^2)$ 对这样的关联累积产生了在 $N \to \infty$ 极限下持续存在的修正。简单平均场之所以达到相当的精度，是因为其熵加权前因子部分吸收了两种方法都忽略的关联效应。
...
我们对皇后格点气体进行了统计力学研究，将大规模蒙特卡洛模拟与解析平均场理论相结合，刻画了 $N$-皇后系统的有限温度热力学。主要发现如下：

\begin{enumerate}
\item 每皇后比热 $C_v/M(T)$ 在 $N \to \infty$ 时收敛到 $T$ 的普适函数。峰值高度 $\approx 1.62$ 不发散，排除了热力学相变，并将交叉行为认定为有效能隙 $\Delta = J$ 的 Schottky 反常。

\item 修正泊松平均场理论——将每条攻击线视为独立的修正泊松变量——给出了 $E/M(T)$ 和 $C_v/M(T)$ 的解析表达式，重现蒙特卡洛数据的精度在 $2$--$7\%$ 以内。残余偏差是 $O(1)$ 的系统效应：每对攻击线通过共享格点产生 $O(1/N)$ 的关联，但 $O(N^2)$ 对这样的关联产生在 $N \to \infty$ 极限下持续存在的累积修正。更简单的熵加权平均场 [$C_v^{\max} = 4(1+\gamma)/e^2$] 在峰值处达到 $1.4\%$ 的精度，这是因为其熵加权前因子部分吸收了关联效应。
...
\item $C_v/T$ 的热力学积分恢复了每皇后基态熵 $s_0^{\rm MC} = S(\infty)/N - \int(C_v/NT)\,dT$，在 $N = 8$ 和 $16$ 时与精确值的吻合度在 $1.2\%$ 以内。对于 $N \ge 32$，提取的 Simkin 常数 $\gamma_{\rm MC} = \ln N - s_0^{\rm MC}$ 向 $\gamma = 1.942$ 单调收敛，在 $N = 128$ 时达到 $1.92$（偏差 $1.1\%$）。这一非平凡的自洽性检验连接了 Simkin 组合公式、高温熵的 Stirling 近似、以及跨越整个温度范围的蒙特卡洛比热数据。

\item 基态熵 $s_0 = \ln N - \gamma$ 可通过约束层次分解：列约束恰好消耗每个皇后 1 个单位的熵（Stirling 修正），两组对角线约束共消耗 $\gamma - 1 \approx 0.94$ 个单位。这一分解在形式上令人联想到理想气体的 Sackur--Tetrode 公式，尽管两者背后的物理机制不同。
...
这些结果将皇后格点气体确立为一个热力学可由独立攻击线平均场理论定量描述的模型系统。泊松平均场方法的解析可处理性、稀薄和稠密填充区域之间丰富的交叉现象学、以及组合量与热力学量之间的相互关联，使该模型成为约束满足与统计力学交叉领域中引人注目的试验场。
