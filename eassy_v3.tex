...
precisely encoded using the matrix product operator (MPO) formalism.
...
projects onto the occupied state (one queen). The local tensor $A$ of MPO is
\begin{equation}
  A = \begin{pmatrix}
    \hat{n}_0 & \hat{n}_1 \\
    0         & \hat{n}_0  
...
any (anti-)diagonal can also be encoded as a MPO
\begin{align}
  &\,\,\,\,\,\,\,\, \hat{n}_1 \otimes \hat{n}_0 \otimes \hat{n}_0 
   + \hat{n}_0 \otimes \hat{n}_1 \otimes \hat{n}_0 
   + \hat{n}_0 \otimes \hat{n}_0 \otimes \hat{n}_1 \nonumber \\
...
a rank-9 local tensor $T$ at each site, namely,
\begin{equation}
  T_{ud,\,lr,\,\bar{u}\bar{d},\,\bar{l}\bar{r}}^{\,\alpha}
...
$\nearrow$-diagonal directions, and $\alpha,\alpha',\beta',\beta,\sigma \in\{0,1\}$ are the physical
indices.  Since the local tensor $A_{ij}^{\sigma\sigma'}$ of MPO has only three nonzero elements, namely,
$A_{00}^{00} = A_{11}^{00} = 1$ (empty site: signal pass-through)
and $A_{01}^{11} = 1$ (occupied site: emit attack signal), the local tensor $T$ of the ground state 
...
of solutions $Q(N)$ can be obtained by contracting a tensor network with $D=2$ 
bond dimension. Figure~\ref{fig:tn_network} illustrates the complete tensor network
...
problems~\cite{Kourtis2019}, of which the $N$-queens problem is a
natural instance.
The tensor network can be exactly contracted row by row using boundary matrix product state methods~\cite{Xiang2024}, with a
computational cost that grows exponentially in $N$ through the bond
dimension $D$ but is free of statistical noise, providing exact
benchmarks that complement the Monte Carlo results at moderate~$N$.

Another perspective on the tensor network contraction problem is to 
formulate it as a row-by-row transfer matrix $\mathcal{T}$ raised to 
the $N$-th power. This yields 
\begin{equation}
  Q(N) = \mathrm{tr}\big(\mathcal{T}^N\big) = \sum_n \lambda_n^N,
\end{equation}
where $\lambda_n$ are the eigenvalues of the transfer matrix 
$\mathcal{T}$ and satisfy $\lambda_1 > \lambda_2 \geq \dots \geq \lambda_N$. 
When $N$ is sufficiently large and the leading eigenvalue is unique, 
the number of solutions is dominated by the leading term:
$Q(N) \sim \lambda_1^N$.
For classical statistical models such as the two-dimensional Ising 
model, the partition function can also be written as a tensor network, 
and the largest eigenvalue $\lambda_1$ of its transfer matrix is unique 
and scales as $\alpha^N$ for some constant $\alpha$. Thus, the partition 
function $Z \sim (\alpha^N)^N = \alpha^{N \times N}$, and the free 
energy $F \sim \ln Z \sim N \times N \ln \alpha$, demonstrating that 
it is an extensive quantity. For the $N$-queens problem, as established 
above, $Q(N) \sim (N/e^{\gamma})^N$, so the leading eigenvalue scales as 
$\lambda_1 \sim O(N)$, rather than the typical $O(\alpha^N)$ in standard 
statistical models. This means that, compared to usual classical
statistical models, the growth rate of the largest eigenvalue of
the transfer matrix for the $N$-queens problem is much slower.
Therefore, if one uses standard contraction techniques for the 
tensor network of the $N$-queens problem, such as CTMRG or boundary MPS 
methods, the result will converge to a trivial value
$\alpha = 0$ in the $N \to \infty$ limit.

However, the fact that $\lambda_1 \sim O(N)$ suggests that the
$N$-queens problem is more akin to a ground-state problem with a Hamiltonian
$H = -\mathcal{T}$ and , rather than a
conventional partition function problem. Therefore, we can use 
standard ground-state algorithms such as DMRG or VUMPS to 
compute the ground-state energy $E_0 = -e^{-\gamma}$ of the 
Hamiltonian $H = -\mathcal{T}$. Eventually, we can obtain the 
Simkin constant $\gamma = -\rm{ln}(-E_0)$.
...
\begin{figure}[t]
  ...
  Solid red lines are row bonds and solid
  ...
  allowed).  Contracting the full network involves summing over all
  bond indices and all physical indices; the latter summation is
  equivalent to contracting each physical leg with the vector
  $(1,1)^T = |0\rangle + |1\rangle$, which enumerates all $2^{N^2}$
  occupation configurations.  The result is the exact solution count
  $Q(N)$.  The arrow indicates the row-by-row contraction order used
  in the matrix product operator approach.}
...
These results establish the $N$-queens lattice gas as a bridge
between combinatorics and statistical mechanics: a single Monte Carlo
campaign can determine a fundamental combinatorial constant---the
Simkin constant $\gamma$---through the laws of thermodynamics.
