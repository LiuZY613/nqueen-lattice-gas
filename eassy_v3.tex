...
further narrow the gap. \lw{try to carry out finite size scaling analysis, 
or at least make a gamma versus N plot, indicate the Nobel, A. Agrawal, and S. Boyd value there}
...
$\langle v_f|$ gives \lw{that is the relation of $v_{0,f}$ and  $\mathbf{v}_{0,1,2}$ 
  ?}
\begin{equation}
  Q(N) = \langle v_f |\,\mathcal{T}^N\,| v_0 \rangle,
\end{equation}
where $|v_0\rangle$ encodes the initial (no queen seen) boundary
condition and $\langle v_f|$ projects onto the state in which every
column is occupied exactly once.
