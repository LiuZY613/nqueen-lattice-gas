...
We also present a complementary exact approach based on tensor
networks.  Each non-attacking constraint---row, column, or
...
Section~\ref{sec:conclusions} concludes with a discussion of how the Monte Carlo and tensor network approaches complement each other.
...
precisely encoded using the matrix product operator (MPO) formalism~\cite{Crosswhite2010}.
...
$\mathcal{T}$ and satisfy $\lambda_1 > \lambda_2 \geq \dots$. 
...
function $Z \sim (\alpha^N)^N = \alpha^{N^2}$, and the free 
energy $F \sim \ln Z \sim N^2 \ln \alpha$, demonstrating that 
...
statistical models. This anomalous scaling distinguishes the $N$-queens counting problem
from conventional partition functions: standard infinite-size
contraction techniques such as CTMRG~\cite{Nishino1996} or boundary MPS methods would
yield a trivial value $\alpha = 0$ in the $N \to \infty$ limit.
Consequently, finite-$N$ exact contraction remains the natural
framework for obtaining tensor network benchmarks, as described above.
...
without statistical noise at moderate~$N$, while the Monte Carlo
approach reaches much larger system sizes.  Together, the two methods
cover complementary regimes of~$N$.  An open direction is to
exploit this synergy to probe the finite-size corrections to
the Simkin formula and further sharpen the infinite-$N$ extrapolation
of~$\gamma$.
...
\bibitem{Crosswhite2010}
G.~M.~Crosswhite, A.~C.~Doherty, and G.~Vidal,
